...
light_thresh = 120; % threshold for light to be detected in that coordinate
...
Turn=0
for i=1:ncount
    [xpos,ypos,Turn]= FindNextPoint(xpos,ypos,x_delta,y_delta,foundpts,max_light,light_thresh,Turn,gain,exposure,vid,mMTIDevice);
    disp('Line light found'); 
    %max_MEMS=[xpos,ypos];
    %max_light=light_thresh;
    %maxsnap=snap;
    %nearpts1=gridmatrixNEW([max_MEMS(1)-nearng max_MEMS(1)+nearng],[max_MEMS(2)-nearng max_MEMS(2)+nearng],6,minsep,0); 
